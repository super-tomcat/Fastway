...
(defcustom fastway-overlay-default-choice 1
  "Use this option to set Fastways default highlighting overlay. In other words how
the matches are highlighted when Fastway is first switched on. You can only choose one
face out of three, or you can elect to have no overlay which means no highlighting."
  :type '(choice (const :tag "Fastway Matches Overlay 1" 1)
                 (const :tag "Fastway Matches Overlay 2" 2)
                 (const :tag "Fastway Matches Overlay 3" 3)
                 (const :tag "No Overlay" 4)) 
  :group 'fastway)
;; 
